...
def train_model(
    ...
    ext = '_t' + str(thresh) + '_m_' + str(mean)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for phase in ['train', 'val']:
            ...
            for data in dataloaders[phase]:
                ...
                try:
                    inputs = C.crop_batch(inputs,global_model,thresh,method,mean = mean)
                except:
...
def train_cnn(PATH_TO_IMAGES, LR, WEIGHT_DECAY,state_dict,heatmap_methods, paths):
    ...
    global_model = model
    global_model.load_state_dict(torch.load(state_dict))    
    ...
    # train model
    model, best_epoch = train_model(model, criterion, optimizer, LR, global_model, heatmap_methods,
                                    num_epochs=NUM_EPOCHS,
                                    dataloaders=dataloaders, dataset_sizes=dataset_sizes, weight_decay=WEIGHT_DECAY)

    # get preds and AUCs on test fold
    preds, aucs = E.make_pred_multilabel(
        data_transforms, model, global_model, PATH_TO_IMAGES, heatmap_methods, paths)
